@cannot('inscribe', $course)
    @can('review', $course)
        <div class="col-12 pt-0 mt-4 text-center">
            <h2 class="text-muted">{{ __('app.courses.review_form_title') }}</h2>
            <hr>
        </div>
        <div class="container-fluid">
            <form action="{{ route('courses.add_review') }}" method="post"
                  class="form-inline" id="rating_form">
                @csrf
                ...
                <div class="form-group">
                    <div class="col-12">
                        <textarea name="message" id="message"
                                  cols="120" rows="8"
                                  placeholder="{{ __('app.courses.review_form_textarea') }}"
                        ></textarea>
                    </div>
                </div>
                <button type="submit" class="btn btn-warning text-white">
                    <i class="fa fa-space-shuttle">{{ __('app.courses.review_form_button') }}</i>
                </button>
            </form>
        </div>
    @endcan
